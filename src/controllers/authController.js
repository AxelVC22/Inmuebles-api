...
const registerUser = async (req, res) => {
  const { nombre, apellidos, email, password, rol, telefono, fechaNacimiento, nacionalidad } =
    req.body;
  ...
  try {
    ...
    if (rol === 'Cliente') {
      ...
      if (!req.body.rfc)
        return res.status(400).json({ error: 'El RFC es obligatorio para Arrendadores' });
