...
const updateUserPreferences = async (req, res, next) => {
    try {
        ...
        if (presupuestoMin && isNaN(presupuestoMin)) return res.status(400).json({error: 'Presupuesto mínimo inválido'});
        if (presupuestoMax && isNaN(presupuestoMax)) return res.status(400).json({error: 'Presupuesto máximo inválido'});
